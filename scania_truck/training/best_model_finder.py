...
from scania_truck.utils.helper import log_and_load_production_model, mlflow_model_logger
        ...
        try:
            ...
            for i in clusters:
                ...
                FileManager.save_model(
                    model=prod_model,
                    model_path=f"scania_truck/models/{model_name}_{str(id)}.pckle",
                )
    ...
    def predict(self, X: pd.DataFrame, y: pd.Series = None):
        ...
        try:
            ...
            kmeans = FileManager.load_model(
                model_path=config.app_config.kmeans_model_path
            )
            ...
            for i in clusters:
                _X = X[X["Clusters"] == i]
                _X = _X.drop("Clusters", axis=1)
                for model_name in os.listdir(config.app_config.prod_model_path):
                    if model_name.endswith(f"_{str(i+1)}.pckle"):
                        model = FileManager.load_model(
                            model_path=f"{config.app_config.prod_model_path}{model_name}"
                        )
